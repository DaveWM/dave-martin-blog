...
Ideally, we want to be able to pass an arbitrary query to the database and be notified when the result changes. Unfortunately, very few databases offer this. The solution mentioned in "The Web After Tomorrow" is to create a "reversible" query language - one that can be used to query the database, but also to determine if a database transaction affects the query results. Although there has been some progress on this front, it's sadly missing from all mainstream databases.
